...
func startCacheServe(addr string, addrs []string, gee *geecache.Group) {
	pees := geecache.NewHTTPPool(addr)
	pees.Set(addrs...)
	gee.RegisterPeers(pees)
	log.Println("geecache is running at", addr)
	log.Fatal(http.ListenAndServe(addr[7:], pees))
}
